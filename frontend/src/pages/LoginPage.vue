...
const performLivenessCheck = async () => {
  ...
  const checkLoop = async () => {
    ...
    if (prevFrame && isSpoofed(landmarks, ctx, prevFrame, currentFrame, canvas.width, canvas.height)) {
      message.value = 'Face not detected successfully. Try again!.';
      messageColor.value = 'error';
      loading.value = false;
      return;
    }
    ...
    if (passed) {
      ...
      message.value = 'Now Authenticate!';
...
const authenticateFace = async () => {
  ...
  try {
    ...
    if (detections.length > 1) {
      message.value = 'Multiple faces detected. Only one face is allowed!';
      messageColor.value = 'error';
      return;
    }
...
const authenticateFingerprint = async () => {
  try {
    ...
    if (cred) {
      message.value = 'Fingerprint recognized successfully...!';
      messageColor.value = 'success';
      setTimeout(() => router.push('/dashboard'), 1500);
    } else {
      message.value = 'Fingerprint not recognized. Please try again!';
      messageColor.value = 'error';
    }
  } catch {
    message.value = 'Your fingerprint couldnt be recognized...!';
    messageColor.value = 'error';
  }
};
...
.navbar {
  background-image: url('frontend/src/assets/th.jpg'); /* replace with your path */
